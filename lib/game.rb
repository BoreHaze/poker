...
class Game
  ...
  def showdown
    ...
    players_in_hand.each do |player|
      wins = true
      puts "#{player.name} reveals a #{player.player_hand_rank}"
    ...
    if winners.count == 1
      puts "#{winners[0].name} wins the hand with a #{winners[0].player_hand_rank} "
    else
      winner_str = ""
      ...
      puts "#{winner_str} tie for the hand with a #{winners[0].player_hand_rank}"
    end
